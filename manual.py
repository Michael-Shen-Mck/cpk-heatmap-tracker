import streamlit as st


def render_user_manual() -> None:
    _, manual_col = st.columns([0.78, 0.22])
    with manual_col:
        with st.popover("打开使用说明书", use_container_width=True):
            st.markdown(
                """
                ### 1. 规格配置

                - 先维护规格、目标捆重和允许负差百分比。
                - 允许负差填写百分比，例如目标 2000kg、允许负差 0.4%，系统会自动计算最低允许捆重为 1992kg。
                - 停用规格不会出现在录入页面，但历史数据仍可查询。

                ### 2. 数据录入

                - 选择规格、生产日期、轧线、班组、批号和录入人。
                - 轧线可选三轧/四轧，班组可选甲班/乙班。
                - 手工录入时，捆重可以一行一个，也可以补充每捆的炉号、捆号和备注。
                - Excel 导入时，先下载模板；模板里一行代表一捆，包含炉号、捆号、捆重和备注，同一批号填写多行即可自动合并成一个批次。
                - 提交前先看预览，确认样本数、平均捆重、平均偏差和负差数。

                ### 3. 热力图查看

                - 选择年份和月份后，系统按“规格 x 日期”展示 Cpl。
                - 可以按轧线和班组筛选，查看三轧/四轧、甲班/乙班的表现。
                - 绿色表示 `Cpl >= 1.33`，黄色表示 `1.00 <= Cpl < 1.33`，红色表示 `Cpl < 1.00`。
                - 样本不足 5 条时不计算 Cpl。

                ### 4. 数据导出和纠错

                - 在“数据明细与导出”中可以导出捆重明细、批次汇总、每日指标和区间汇总。
                - 如果录错批次，可以在“批次汇总”页签中勾选确认后删除整个批次。

                ### 5. 计算口径

                - 偏差 = 实际捆重 - 目标捆重
                - 允许负差重量 = 目标捆重 x 允许负差百分比
                - 允许负差下限 = -允许负差重量
                - Cpl = (平均偏差 - 允许负差下限) / (3 x 偏差标准差)
                """
            )
